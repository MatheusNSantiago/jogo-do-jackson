  ...
  create() {
    ...
    if (this.mensagem === 'Você Venceu!!!') {
      this.add.text(
        150,
        50,
        'Você venceu com o tempo de: ' + game.scene.keys['PlayGame'].tempo,
        { fontSize: '22px', fill: 'purple' }
      );
      if (localStorage.getItem('recorde') < game.scene.keys['PlayGame'].tempo) {
        ...
        this.add.text(150, 80, 'Parabéns, você tem o novo recorde do jogo', {
          fontSize: '22px',
          fill: 'purple',
        });
      }
    } else {
      this.add.text(150, 50, this.mensagem, {
        fontSize: '22px',
        fill: 'purple',
      });
    }
